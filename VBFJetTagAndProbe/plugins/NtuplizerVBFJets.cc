...
// Class:      NtuplizerVBFJets
// 
/**\class NtuplizerVBFJets NtuplizerVBFJets.cc QCDTagAndProbe/QCDTagAndProbe/plugins/NtuplizerVBFJets.cc
...
#ifndef NTUPLIZERVBFJets_H
#define NTUPLIZERVBFJets_H
...
#define NUMBER_OF_MAXIMUM_TRIGGERS 2
//
// class declaration
//
...
class NtuplizerVBFJets : public edm::EDAnalyzer {
   public:
      explicit NtuplizerVBFJets(const edm::ParameterSet&);
      virtual ~NtuplizerVBFJets();
   ...
   private:
      ...
      unsigned long jetTriggerBits_;
      unsigned long jetTriggerBits_1_;
      unsigned long jetTriggerBits_2_;
      float tauPt_1_;
      float tauEta_1_;
      float tauPhi_1_;
      int tauCharge_1_;
      ...
      std::bitset<NUMBER_OF_MAXIMUM_TRIGGERS> jetTriggerBitSet_;
      std::bitset<NUMBER_OF_MAXIMUM_TRIGGERS> jetTriggerBitSet_1_;
      std::bitset<NUMBER_OF_MAXIMUM_TRIGGERS> jetTriggerBitSet_2_;
...
//
NtuplizerVBFJets::NtuplizerVBFJets(const edm::ParameterSet& iConfig) :
...
NtuplizerVBFJets::~NtuplizerVBFJets()
{
    return;
}
...
void NtuplizerVBFJets::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    Initialize();
    ...
    bool foundDiTauTriggerLead = false;
    bool foundDiTauTriggerTrail = false;
    jetTriggerBitSet_1_.reset();
    jetTriggerBitSet_2_.reset();
    jetTriggerBitSet_.reset();
    for (pat::TriggerObjectStandAlone obj: *triggerObjects)
    {
        obj.unpackPathNames(names);
        const edm::TriggerNames::Strings& triggerNames = names.triggerNames();
        if (obj.hasTriggerObjectType(trigger::TriggerTau))
        {
            ...
            if (dR2 < 0.5)
            {
                ...
                for (const tParameterSet& parameter : parameters_Tag_)
                {
                    if ((parameter.hltPathIndex >= 0) && obj.hasPathName(triggerNames[parameter.hltPathIndex], true, false))
                    {
                        foundDiTauTriggerTrail = true;
                    }
                }
            }
        }

        // Perform trigger matching on leading jet
        ...
        if (dRJet1 < 0.5)
        {
	      ...
	      if ((parameter.hltPathIndex >= 0)&&(obj.hasPathName(triggerNames[parameter.hltPathIndex], false, true)))
                {
                    const std::vector<std::string>& filters = (parameter.leg1 == 999)? (parameter.hltFilters1):(parameter.hltFilters2);
                    if (this -> hasFilters(obj, filters))
                    {
                        jetTriggerBitSet_1_[x] = true;
                    }
                }
                x++;
            }
	      ...
	      if ((parameter.hltPathIndex >= 0)&&(obj.hasPathName(triggerNames[parameter.hltPathIndex], false, true)))
                {
                    ...
                    if (this -> hasFilters(obj, filters))
                    {
                        jetTriggerBitSet_2_[x] = true;
                    }
                }
                x++;
            }
        }
        jetTriggerBitSet_[0] = (jetTriggerBitSet_1_[0] && jetTriggerBitSet_2_[1]) || (jetTriggerBitSet_1_[1] && jetTriggerBitSet_2_[0]);
        
        // if (obj.hasPathName("HLT_VBF_DoubleLooseChargedIsoPFTau20_Trk1_eta2p1_Reg_v*", false, false))// && obj.hasTriggerObjectType(trigger::TriggerJet))
        // {
    ...
    jetTriggerBits_ = jetTriggerBitSet_.to_ulong();
    jetTriggerBits_1_ = jetTriggerBitSet_1_.to_ulong();
    jetTriggerBits_2_ = jetTriggerBitSet_2_.to_ulong();
    ...
    // std::cout << "Both taus triggered? " << (foundDiTauTriggerLead && foundDiTauTriggerTrail) << std::endl; 
    // std::cout << "Leading jet pT: " << leadJetPt_ << std::endl;
    // std::cout << "Trailing jet pT: " << trailJetPt_ << std::endl;
    // std::cout << "Mjj: " << Mjj_ << std::endl;
    // std::cout << "Decisions for lead jet:" << jetTriggerBitSet_1_[0] << jetTriggerBitSet_1_[1] << std::endl;
    // std::cout << "Decisions for trail jet:" << jetTriggerBitSet_2_[0] << jetTriggerBitSet_2_[1] << std::endl;
    // std::cout << "Decisions for both jets:" << jetTriggerBitSet_[0] << std::endl;
    // std::cout << std::endl;
    //std::cout << "Fill event with: EventNumber " << indexevents_ << " RunNumber " << runNumber_ << " and LumiSection: " << lumi_ << std::endl;
    if (foundDiTauTriggerLead && foundDiTauTriggerTrail)
    {
        tree_->Fill();
    }
    // else
...
void NtuplizerVBFJets::beginJob()
{
    edm::Service<TFileService> fs;
    ...
    tree_->Branch("jetTriggerBits", &jetTriggerBits_, "jetTriggerBits/l");
    tree_->Branch("leadJetTriggerBits", &jetTriggerBits_1_, "leadJetTriggerBits/l");
    tree_->Branch("trailJetTriggerBits", &jetTriggerBits_2_, "trailJetTriggerBits/l");
...
void NtuplizerVBFJets::endJob() 
{
    return;
}


void NtuplizerVBFJets::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{
    ...
    if(!hltPrescale_->init(iRun, iSetup, processName_.process(), changedConfig)){
...
void NtuplizerVBFJets::Initialize()
{
    indexevents_ = 0;
    runNumber_ = 0;
    lumi_ = 0;
    ...
    jetTriggerBits_ = 999;
    jetTriggerBits_1_ = 999;
    jetTriggerBits_2_ = 999;
    leadJetPt_ = -1.;
    leadJetEta_ = -999;
    leadJetPhi_ = -999;
...
void NtuplizerVBFJets::endRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{
    return;
}

bool NtuplizerVBFJets::hasFilters(const pat::TriggerObjectStandAlone&  obj , const std::vector<std::string>& filtersToLookFor)
{
...
int NtuplizerVBFJets::GenIndex(const pat::TauRef& tau, const edm::View<pat::GenericParticle>* genparts)
{
    float dRmin = 1.0;
    int genMatchInd = -1;
...
DEFINE_FWK_MODULE(NtuplizerVBFJets);
#endif
